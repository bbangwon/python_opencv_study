...
# BGRA
img_plus = cv2.add(img, (50, 50, 50, 0)) # cv2연산은 saturated 연산이어서 255를 넘어가면 255로 고정된다.
img_minus = cv2.subtract(img, (50, 50, 50, 0)) # cv2연산은 saturated 연산이어서 0보다 작아지면 0으로 고정된다.
...
cv2.imshow('ResultsMul', img_mul)
cv2.imshow('ResultsDiv', img_div)
